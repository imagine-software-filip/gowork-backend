...
class JobService(AbstractBaseModel):
    """Each freelancing posts can have several different services"""
    HOUR = "HOUR"
    ONCE = "ONCE"
    PAYMENT_CHOICES = (
    ...
    job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name="service_job")
    service_name = models.CharField(max_length=255)
    payment_type = models.CharField(max_length=10, choices=PAYMENT_CHOICES)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    def __str__(self):
        """Str representation of the object"""
        return str(f"{self.job.user.email} - {self.service_name}")
...
class Offer(AbstractBaseModel):
    ...
    requester = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="requester_user")
    provider = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="provider_user")
    ...
    def __str__(self):
        return f"Request: {self.requester.email} | Provider: {self.provider.email}"


class ResolvedService(AbstractBaseModel):
    offer = models.ForeignKey(Offer, on_delete=models.CASCADE, related_name="resolvedservice_offer")
    provider = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="resolvedservice_provider")
    requester = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="resolvedservice_requester")
